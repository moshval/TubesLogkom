...
w :- posX(X), posY(Y), Y2 is Y-1, isStore(X,Y2), write('Write `store.` to see store menu.'), retract(posY(_)), asserta(posY(Y2)), !.
...
s :- posX(X), posY(Y), Y2 is Y+1, isStore(X,Y2), write('Write `store.` to see store menu.'), retract(posY(_)), asserta(posY(Y2)), !.
...
a :- posX(X), posY(Y), X2 is X-1, isStore(X2,Y), write('Write `store.` to see store menu.'), retract(posX(_)), asserta(posX(X2)), !.
